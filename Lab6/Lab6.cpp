...
//create pointers that will be maniuplated 
player *head = NULL;
player *ptr = NULL; 
player *sort = NULL; 
...
void createplayer(string first, string last, int year){ 
...
void populate(){
  ...
  int element = 0;
  ifstream infile("Lab6_QB_List.txt");//function to get information from text file
  string last; 
  string first;
  int year;
  ...
  while(!infile.eof()) {
   ...
   //if the first element 
    ...
    //intialize counters and indexes
      ...
      player *temp; 
      temp = head;//set temp equal to the beginnin of the list 
...
void deleteplayer(){
  ...
  int found = 0;
//if name entered equals name in list
  if(head->first == first){
        if(head->last == last){ 
...
cout << current->first << " " << current->last << " ";
for(int i = 0; i < current->numWin; i++){
    cout << current->year[i] << " ";
}
cout << endl;
}
